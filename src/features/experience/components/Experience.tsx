          ...
          {experiences.map((exp, index) => {
            ...
            return (
              <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                {/* Icon */}
                <div className={`flex items-center justify-center w-10 h-10 rounded-full border border-border shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 font-bold font-mono text-xs
                ...
                <motion.div 
                  ...
                  viewport={{ once: true, margin: "-100px" }}
                  className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-card border border-border p-8 rounded-2xl shadow-xl hover:border-accent/50 transition-colors flex flex-col gap-6"
                >
